...
public class Client {
  ...
  public static void main(String[] args) {
    try {
      ...
      System.out.println("ИНФО: Подключились к серверу: tcp://localhost:" + Server.PORT);
      ...
      // Отправили идентфикатор на сервер
      ...
      throw new RuntimeException("ИНФО: Не удалось подключиться к серверу: " + e.getMessage(), e);
    }
  }
...
class ServerWriter implements Runnable {
  ...
  public void run() {
    ...
    try (PrintWriter out = new PrintWriter(serverSocket.getOutputStream(), true)) {
      while (true) {
        ...
        if (Objects.equals("/exit", msgFromConsole)) {
          System.out.println("ИНФО: Отключаемся...");
          break;
        }
      }
    } catch (IOException e) {
      System.err.println("ОШИБКА: Ошибка при отправке на сервер: " + e.getMessage());
    }
    ...
    try {
      ...
      System.err.println("ОШИБКА: Ошибка при отключении от сервера: " + e.getMessage());
    }
  }
...
class ServerReader implements Runnable {
  ...
  public void run() {
    try (Scanner in = new Scanner(serverSocket.getInputStream())) {
      ...
      System.err.println("ОШИБКА: Ошибка при отключении чтении с сервера: " + e.getMessage());
    }
